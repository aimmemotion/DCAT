...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for fullImg, mipImg, targets in metric_logger.log_every(data_loader, print_freq, header):
        batch_size = targets.size(0)

        fullImg = fullImg.to(device, non_blocking=True)
        #print("fullImg.shape = {}".format(fullImg.shape))
        mipImg = mipImg.to(device, non_blocking=True)
        #print("mipImg.shape = {}".format(mipImg.shape))
        ...
        if mixup_fn is not None:
            #print("mixup_fn")
            fullImg, targets = mixup_fn(fullImg, targets)
            #mipImg, targets = mixup_fn(fullImg, targets)
        

        with torch.cuda.amp.autocast(enabled=amp):
            outputs = model(fullImg, mipImg)
...
def evaluate(data_loader, model, device, world_size, distributed=True, amp=False):
    ...
    for fullImg, mipImg, target in metric_logger.log_every(data_loader, 10, header):
        fullImg = fullImg.to(device, non_blocking=True)
        mipImg = mipImg.to(device, non_blocking=True)
        target = target.to(device, non_blocking=True)
        # compute output
        with torch.cuda.amp.autocast(enabled=amp):
            output = model(fullImg, mipImg)
    ...
    print("targets.shape = {}, len = {}".format(targets[0].shape, len(targets)))
    print("outputs.shape = {}, len = {}".format(outputs[0].shape, len(outputs)))
    
    outputs = torch.cat(outputs, dim=0)
    targets = torch.cat(targets, dim=0)
    
    real_acc1, real_acc3 = accuracy(outputs[:num_data], targets[:num_data], topk=(1, 3))
    ...
    metric_logger.meters['acc3'].update(real_acc3.item())
